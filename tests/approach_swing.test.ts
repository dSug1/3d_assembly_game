...
describe("⛔⛔⛔ AN APPROACH ALONG GRAVITY ARMS THE SWING — the 2026-09-23 device report", () => {
  /**
   * ⚠⚠ *"The swing of camera [is] missing at offset radius zone enter sometimes when the follower
   * approaches the pioneer from the gravity axis direction."*
   *
   * ⛔⛔ **THE RULE WAS NEVER WRONG — IT WAS NEVER FED.** `swingSignFor` has always answered `1`
   * for a purely vertical travel, with a comment saying so. ⚠ The scene accumulated the travel in
   * the HOLDER's branch only, and the gravity axis is the SECOND touchpoint's channel — which
   * applied its displacement and recorded nothing. ⭐ So the arming call was
   * `swingSignFor(0, 0)` — `null`, no swing — and *"sometimes"* was exactly the frames where the
   * holder happened to be moving too.
   *
   ...
   * function applying the step AND recording it, because the missing line was the symptom while
