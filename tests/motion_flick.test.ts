...
/**
 * ⭐⭐⭐ **DEFECT 70 — A SPEED THAT NEVER DECAYS, AND THE REPORT THAT NAMED IT.**
 *
 ...
 * approach swing, which reads it at the START of an approach and **latches** the answer.
 *
 * ⭐ So an immediate second push inherits the first one's speed and the swing is damped to a
 * degree or two; pause longer than one window and the reading is honest again.
 */
describe("⛔⛔⛔ defect 70 — the speed window may end NOW instead of at the last sample", () => {
  ...
  it("⛔ and the RELEASE reading is untouched — the default is the old behaviour exactly", () => {
    // ⚠ The flick asks at the release, where `now` and the last sample are the same instant.
    expect(terminalSpeedPxPerS(trimBuffer(fast, cfg, 1032), cfg)).toBe(
      terminalSpeedPxPerS(trimBuffer(fast, cfg), cfg),
    );
    // ⭐ A `now` inside the window keeps the samples that are still inside it.
