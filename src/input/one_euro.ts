 ...
 *
 * ⛔⛔ IT WAS REVERTED ONCE, ON BAD EVIDENCE, AND THAT IS WHY IT IS BACK.
 * On 2026-09-14 it measured no improvement and was removed under `METHOD`'s
 * measure-or-revert rule. ⚠ **Those measurements were taken on PERFECT-CIRCLE
 * fixtures** — the very fixtures that, hours later, turned out to be why roll had
 * vanished from the device entirely. The estimator looked clean on them because they
 * were idealised, so there was nothing left for a filter to improve. The null result
 * was measured on invalid specimens and did not mean what it appeared to.
 * ⭐ Re-measured against IMPERFECT gestures — ellipses with drifting centres — and
 * against the Hyper estimator that replaced Kåsa. The numbers are in
 * `Claude/00_CORE/queue_notes/IN1.md`; the owner's device preference agreed.
 *
