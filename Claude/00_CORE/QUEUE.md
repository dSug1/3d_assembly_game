...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **1158 golden vectors,
all passing** (37 → … → **1158**; ⭐ each DROP has a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE.**
...
⭐⭐ **THE HUD HAS A `jump` LINE** (`input/jump_watch.ts`): a body's model pose is watched every frame and any **discontinuity** is latched with the verdict in force. ⚠⚠ **NOT an open defect** — one was reported 2026-09-25 and withdrawn. ⭐ It stays because finding nothing cost a re-read of every path that could cause a jump; that list is the module's header.
⚠ **THE HOLLOW CYLINDER IS REMOVED** (`D92`, reversed the same day) — `objectD` is the `L × 2L × 3L` pink cuboid again, and `core/ring.ts` with its 10 vectors is **deleted, not parked**. ⭐ Defect 68 is what the hour bought: the mesh was correct and it still read as a doughnut, because the rim vertices are shared and `ComputeNormals` blends across them.
...
⭐⭐ **A MOUSE IS TWO TOUCHPOINTS NOW** (`D94`): **LMB = #1, RMB = #2**, the cursor drives the most recently pressed and the other **parks**, wheel = a synthesised pinch, Esc lifts everything. ⛔ A TRANSLATION, not a second input path — `input/desktop_pointers.ts` is pure and no rule knows it exists, so deleting one import and one call in `scene.ts` leaves the touch build identical. ⚠ Only `pointerType === "mouse"` is intercepted, so a finger is untouched and no flag is needed. ⚠⚠ **Playable, NEVER testable** — rule 5 needs a finger, and every threshold it drives was judged by one.
