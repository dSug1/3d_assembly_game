...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **1168 golden vectors,
all passing** (37 → … → **1168**; ⭐ each DROP has a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE.**
...
⭐⭐ **THE HUD HAS A `jump` LINE** (`input/jump_watch.ts`): a body's model pose is watched every frame and any **discontinuity** is latched with the verdict in force. ⚠⚠ **NOT an open defect** — one was reported 2026-09-25 and **withdrawn**, and its ledger entry went with it. ⭐ It stays because finding nothing cost a re-read of every path that could cause a jump; that list is the module's header. ⭐⭐ A jump is a step large against the body's OWN recent median, never a constant.
