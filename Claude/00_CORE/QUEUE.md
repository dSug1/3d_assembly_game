...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **1136 golden vectors,
all passing** (37 → … → **1136**; ⭐ each DROP has a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE.**
...
⛔⛔⛔ **THE PRODUCT CHANGED SHAPE AND THE WHOLE SUITE STAYED GREEN** (defect 66, 2026-09-25): two test files kept their **own retyped copy** of the body dimensions, so scaling the pyramid (`D91`) moved the product and not the fixtures — including the vector whose stated job is the boot clearance. ⭐⭐⭐ *A fixture that mirrors a constant is a second implementation of it, and it disagrees exactly when the constant is what changed.* ✅ One home: `core/scene_dims.ts`, and three assertions went RED the moment they were wired to it.
⭐⭐ **AND THE FOURTH PART IS A BEVELLED HOLLOW CYLINDER** (`D92`): `L` thick about its own z, outer radius `L`, hole `L/2`. ⭐⭐ *"You did not do a cylinder, you did a doughnut"* was about the **SHADING** — wall and end face share their rim vertices, so `ComputeNormals` blended them; a narrow chamfer keeps the rim crisp while the wall stays smooth. ⛔ **First curved body** — 1° grouping gives it **146** logical faces against a cuboid's 6, and its hole's wall is the first outward normal that is not *away from the centre*. ⚠⚠ The hole is **invisible to the capture**, which is a convex hull (`D49`) → `core/ring.ts`
