...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **1160 golden vectors,
all passing** (37 → … → **1160**; ⭐ each DROP has a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE.**
...
⭐⭐ **THE HUD HAS A `jump` LINE** (`input/jump_watch.ts`) and a `desktop` one: a discontinuity in any body's model pose is latched with the verdict in force, and `seen → sent → got` counts the desktop chain at opposite ends. ⚠⚠ Neither is an open defect.
⚠ **THE HOLLOW CYLINDER IS REMOVED** (`D92`, reversed the same day). ⭐ Defect 68 is what the hour bought: the mesh was correct and it still read as a doughnut, because the rim vertices are shared and `ComputeNormals` blends across them.
⭐⭐ **THE SCENE BOOTS UNALIGNED AND IN `TRANSLATE`** (`D93`, 2026-09-25), with the **two parts tilted 30°** in opposite senses — roll about world `z`, pitch about world `x` (`A7`'s axes at the boot camera), roll first. ⛔ `bootAlignment` is **deleted**; it never rotated anything, so only the STATE changed. ⚠⚠ **`D63`'s jig goes with it** — the swing trial opened pre-aligned, and its earlier verdicts are not comparable with later ones. ⚠⚠ **Measured cost of the tilt**: the pyramid's gap to the plate closes to **53 mm** against a **60 mm** band, so **that pair shows a white capture contour at boot**; the parts' mutual gap moved 280 → 216 mm.
⭐⭐ **A MOUSE IS THE TOUCHPOINT IT LACKS** (`D94`): **LMB stays the browser's own pointer**, RMB is #2, `Shift`+drag moves #2, wheel = a synthesised pinch, Esc lifts the synthetic ones. ⛔⛔ **The first build intercepted EVERYTHING and froze the desktop** (defect 71) — a mouse was already touchpoint #1. ⭐⭐⭐ *Pass through by default, intercept by exception*, and **the blast radius of a layer is the set of events it swallows** — `suppress` is asserted on every path. ⛔ A TRANSLATION, not a second input path — `input/desktop_pointers.ts` is pure and no rule knows it exists, so deleting one import and one call in `scene.ts` leaves the touch build identical. ⚠ Only `pointerType === "mouse"` is intercepted, so a finger is untouched and no flag is needed. ⚠⚠ **Playable, NEVER testable** — rule 5 needs a finger, and every threshold it drives was judged by one.
...
| IN3 | Rules 1–3 (one touchpoint): select, free rotate, flick-to-align, roll, constrained rotate | IN | feature | ✅✅ **THE ALIGNMENT MODEL IS CLOSED BY A DEVICE LOOK (2026-09-17)** — tap-to-align (**anti-parallel** since `D78`, capped at one, `SNAPSHOT`/`FOLLOW`), both faces marked, the twist, two undos, an eased slerp. ⚠ The corrections have not been re-judged. ⛔ **NOT built**: `TargetPosition`, its gizmo, the orbit, the approach. ✅✅ `D73`’s increments closed 2026-09-22, shipping at **0**. ⛔ **`D87`–`D90` (the fuchsia offer, the inverted roles, the swap) are BUILT AND UNJUDGED** — spec §11 → [`queue_notes/IN3.md`](queue_notes/IN3.md) | IN1, 3D1 |
| IN4 | Rules 4–6 (two touchpoints): zoom, translate, mutual approach, mate flick | IN | feature | ✅✅ **RULE 6 CLOSED 2026-09-15** — by finger, its gain **computed**. ⛔⛔ **ITS SCREEN-PLANE FORM IS SUPERSEDED** (`D75`/`D76`): a body translates along **its own axes**, the finger’s delta solved onto both horizontal ones. ⚠ Unjudged again, and `screenTranslation`/`depthTranslate` stay as declared debt. ⛔ 6bis onward still wait on face centres → [`queue_notes/IN4.md`](queue_notes/IN4.md) | IN2, 3D1 (6bis onward only) |
| IN5 | ⚠ **MEASURE every config default on a real device.** None is derived | IN | measurement | queued, ⭐⭐ **practical without a rebuild**: every tunable overrides from the URL. ✅ `pointerNoiseMm` = **0.761 mm** is the one number MEASURED. ⛔⛔ **A TRAP**: several tunables are READ but sit OFF the gesture path, so `config_debt` sees them used while they change nothing → [`queue_notes/IN5.md`](queue_notes/IN5.md) | IN3 |
...
| IN8 | ⚠ Two touchpoints on the SAME object — was undefined and reachable (§5) | IN | decision | 🔧 **ANSWERED THREE TIMES AND BUILT.** ⭐ The second touchpoint — inside **or** outside any object — drives **roll by its x** and **depth by its y**. ✅✅ **CLOSED BY A DEVICE LOOK 2026-09-16**, which also closed the small-object hole owed since `A5`. ⛔⛔ `A15`/`D25`’s orphan unselect is **DELETED** (`D54`) → [`queue_notes/IN8.md`](queue_notes/IN8.md) | IN2 |
| IN9 | ⭐ **CAMERA-ONLY rules: 4 (pinch zoom) and 1 (orbit)** — ⛔ needed NO object model | IN | feature | ✅✅ **CLOSED 2026-09-14**, both rules working by finger. Rule 1 cost **three** defects no green suite could see — a **composition nobody had computed** and a scheme **reversed on measurement**. ⭐ *"Three rigs, therefore two transitions"* is enforced by `validateGestureConfig` → [`queue_notes/IN9.md`](queue_notes/IN9.md) | IN1 |
