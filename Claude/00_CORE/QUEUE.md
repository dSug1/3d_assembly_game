...
all passing** (37 → … → 1105 → **1097**; ⭐ the DROPS each have a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE**: *amend the ledger, never a bare number elsewhere.*
...
⭐⭐⭐ **THE GIZMO FLICKER IS FOUND, AND IT WAS `A11`**: the band is pinned to its boundary while moving, so **any reversal re-enters it** and 30 ms later the axis rests — a curving drag blinks 1–4×/s, **the same in case A and case B**. ⛔ Six analyses died on the premise that the two differed. ✅ A BOUNDED hold (`gizmoHoldMs` 200 ms, slider), ⚠ unjudged. ⭐ 38 vectors, six mutants. ⭐ `D77`: **no gizmo on a frozen body** — ⚠ lapsed with `leading_face.ts`, now a guard — and **a second touch on one is a MISS** so it can drive another → [`queue_notes/IN4.md`](queue_notes/IN4.md)
