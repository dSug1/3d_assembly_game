...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **612 golden vectors,
all passing** (37 → 632 → **574** — ⭐ 58 DELETED with the roll channel — → 612). ✅ The engine boundary is enforced by a test.
...
Forty-three defects have been found **by finger** (forty-two of them; one by composing a measurement with a threshold), and **not one was visible to a green
suite**. They are **five** shapes, not forty-three problems — the fifth is below, and it is
...
| IN3 | Rules 1–3 (one touchpoint): select, free rotate, flick-to-align, roll, constrained rotate | IN | feature | 🔨 **IN PROGRESS, fork B of `D29`** (`?anchorRules=1`; fork A = today's behaviour and the default, fork C = **the owner's set, specified and stage-1 BUILT**, `D37`). ✅ **Built and wired**: rule 2's face selection from the picked **NORMAL** and its marker, 2bis's PRECONDITION through `dragRule`, 2ter/2quater's flick→alignment (**only in `ROTATE`**, `D30`), and `A4`'s **eviction shake** as the escape from a full stack (`D32`). ✅✅ **2sexte IS WIRED** (`D34`) — an anchored object twists about **the constraint's own axis**, by drag on one touchpoint or roll on the second, and `A3`'s handover constant turned out to be unnecessary. ✅ The highlight now survives a release while the alignment holds (`D35`), and a flick is read over its TAIL so it fires mid-rotation (`D33`). ⭐⭐ **The row's two defects are the instructive part** and both are in the dossier: a retired gesture that still owned a verdict, and a mode with no way out → [`queue_notes/IN3.md`](queue_notes/IN3.md) | IN1, 3D1 |
