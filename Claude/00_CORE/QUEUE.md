...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **426 golden vectors,
all passing** (37 → 426). ✅ The engine boundary is enforced by a test.
...
Twenty defects have been found **by finger**, and **not one was visible to a green
suite**. They are four shapes, not twenty problems:
...
⛔⛔ **`3D1` IS BUILT (2026-09-15) AND NEXT IS `IN3`.** The object model exists in
`src/core/object_model.ts` with 42 vectors — ⛔ but **BUILT is not CLOSED**: it has no
visible behaviour, so nothing a finger can judge, and it closes when `IN3` wires it.
⭐ `IN3` now has everything it needs: the constraint stack is attached to an object, so rule
