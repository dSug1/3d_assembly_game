...
| IN3 | Rules 1–3 (one touchpoint): select, free rotate, flick-to-align, roll, constrained rotate | IN | feature | 🔨 **IN PROGRESS, in fork B of `D29`'s three anchor-rule forks** (`?anchorRules=1`; fork A = today's behaviour and the default, fork C = the owner's set, inert). ✅ **Rule 2 is built and wired** — face selection from the picked **NORMAL**, not a triangle index (`core/face_pick.ts`, 9 vectors), plus §3 rule 3's unselect. ✅ Built, not wired: `shake.ts` (15) and `anchor_rotate.ts` (25). ⛔ Not built: 2bis's precondition, 2ter/2quater + the flick skip, and the `A3` handover — which `A12` reopened by moving roll to the second touchpoint → [`queue_notes/IN3.md`](queue_notes/IN3.md) | IN1, 3D1 |
