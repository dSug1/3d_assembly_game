...
✅ TypeScript + Babylon + Vite; `npm run verify` = typecheck + **1137 golden vectors,
all passing** (37 → … → **1137**; ⭐ each DROP has a `D` row). ⛔⛔ **A COUNT RESTATED IN TWO PLACES GOES STALE.**
...
⭐⭐ **THE HUD HAS A `jump` LINE** (`input/jump_watch.ts`): a body's model pose is watched every frame and any **discontinuity** is latched with the verdict in force. ⚠⚠ **NOT for an open defect** — one was reported 2026-09-25 and **withdrawn** (*"I cannot reproduce the jump"*), and its ledger entry went with it. ⭐ It stays because finding nothing cost a full re-read of every path that could cause a jump, and that list is the module's header. ⭐⭐ A jump is a step large against the body's OWN recent median, never a constant — a `FOLLOW` follower is *supposed* to move as far as its Pioneer did.
⚠ **THE HOLLOW CYLINDER IS REMOVED** (`D92`, reversed the same day) — `objectD` is the `L × 2L × 3L` pink cuboid again, and `core/ring.ts` with its 10 vectors is **deleted, not parked**. ⭐ Defect 68 is what the hour bought: the mesh was correct and it still read as a doughnut, because the rim vertices are shared and `ComputeNormals` blends across them. *A vector suite that reads the geometry cannot see the shading, and the shading is what a hand judges.*
...
| IN1 | ⭐⭐ The recognizer state machine — PRESSED / COMMITTED_CONTINUOUS / TAP, provisional motion + rollback, release-time priority | IN | feature | ✅ **CLOSED 2026-09-14.** **7 device passes, 14 defects none of which a green suite could see.** ⛔⛔ Its rule **2quinte** roll detector, *"built early and hardened"*, is **DELETED** (`D31`) — no channel since `A12`, and its retired verdict vetoed `IN3`'s flick → [`queue_notes/IN1.md`](queue_notes/IN1.md) | IN0 |
...
| IN8 | ⚠ Two touchpoints on the SAME object — was undefined and reachable (§5) | IN | decision | 🔧 **ANSWERED THREE TIMES AND BUILT.** ⭐ The second touchpoint — inside **or** outside any object — drives **roll by its x** and **depth by its y** (`D22`/`A12`, `A10`). ✅✅ **CLOSED BY A DEVICE LOOK 2026-09-16**, which also closed the small-object hole owed since `A5`. ⛔⛔ **`A15`/`D25`'s orphan unselect is DELETED** (`D54`) → [`queue_notes/IN8.md`](queue_notes/IN8.md) | IN2 |
...
| IN12 | ⭐ **A DEADBAND on the pointer delta** | IN | defect | ✅✅ **CLOSED 2026-09-15 BY `A11`, AND NOT THE WAY THIS ROW SPECIFIED IT** — the owner made §1.1 *itself* a position deadband, so the excess-only travel is computed ONCE. ⭐⭐ **A threshold has a SHAPE as well as a size**: the per-axis band I argued against buys **axis purity**, which no radius can give. ⛔ Still owed: a device pass — `motionDeadbandMm` is the commit threshold, the rest test and the jitter deadband at once → [`queue_notes/IN12.md`](queue_notes/IN12.md) | IN1, IN4 |
