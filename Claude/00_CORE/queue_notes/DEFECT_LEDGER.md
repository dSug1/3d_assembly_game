...
# ⭐⭐⭐ THE GIZMO FLICKER — the full analysis, kept because this cause WILL repeat

> *"I want a flawless and robust logic, not a fix which may have issues later on."* …
> *"capture this analysis in the md files, as this cause for gizmo flicker may repeat later on when
> we build the full game."* — the owner, 2026-09-23

⛔⛔⛔ **SEVEN DEVICE REPORTS, ONE SHAPE, AND FIVE FAILED FIXES.** Read this before building any
readout, marker, highlight or HUD field that reports on a live gesture — the assembly UI will be
full of them.
...
| # | report | what I did |
|---|---|---|
| 1 | gizmo *"flares to big dimensions"* | sized it from the camera |
| 2 | the lines should be full-screen, and only for driven channels | read the CHANNEL, not the travel |
| 3 | *"slight lag… the green line passes through the left face then relocates"* | a 120 ms memory — **rejected**: *"the input itself has no lag"* |
| 4 | *"the gizmo jitters position between faces"* with two fingers | a wider deadband — **rejected**: *"discarded all changes"* |
| 5 | *"competition between the pairs (green, grey) vs. (blue, red) … flicker"* | per-family memory |
| 6 | *"persistence of the grey axis even if the roll is not input"* | ← caused by 5 |
| 7 | *"if the second touch is released, the axis do not disappear immediately"* | ← caused by 5 |

⭐⭐ **EVERY ONE WAS THE SAME MISTAKE AT A DIFFERENT LEVEL**: a property of the instrument derived
from a **per-frame quantity**. Position, then existence, then which lines — and when the noise was
patched with MEMORY, the memory became the next two defects.

## ⭐⭐⭐ Why case B (unaligned + TRANSLATE) never showed it — the owner's own question

He asked it precisely: *"analyze what logic is different between case A and case B."* Three
differences, and each is a mechanism:

**① THE ELIGIBILITY GATE.** It read `isTranslatingMode(grip.mode) || frameTurnAxes.has(id)`.

* In case B `grip.mode` is **always** a translating mode, so the gate never rejects.
* In case A **both operands flip frame to frame**: on an aligned body the second touchpoint's `dy`
  writes `TRANSLATE_2ND` and its `dx` writes `ROTATE` *within one event*, so the mode is whichever
  branch ran last; and `frameTurnAxes` is empty on any frame that finger sent no event.
* ⛔ On those frames the body left the gizmo **entirely** — all six lines, not one pair. That is
  the larger half of what reads as *"flicker"*.

**② WHOLESALE REPLACEMENT ACROSS TWO FAMILIES.** `displayedAxes` replaced the whole six-channel set
whenever any channel was driven. The two families are driven by **two fingers with independent
`A11` deadband clocks**, so in most frames only one had emitted — and it evicted the other.
⚠ Case B has **one** family, so replacement never crosses one. ⭐ Inside a family the same behaviour
is *correct* and is what the owner asked for: *"if there is only dx or only dy, the other gizmo line
should not appear."*

**③ A MEMORY THAT OUTLIVED EVERYTHING.** `gizmoAxes` was never cleared — not on release, not on a
mode toggle. So a stale TRANSLATE set survived into rotation mode and reappeared on any quiet frame.
⚠ That is also the explanation of *"the first time I input both touches the gizmo flickers, but when
I release and input again it disappears"*: the stale set is still in `previous` during the first
gesture, and the first turn-only frame overwrites it for good.

## ⭐⭐⭐ The fix, and why it is structural rather than another patch

> **Every input the instrument reads is a STATE with a defined lifetime. Nothing is an accumulator,
> a memory, or the arrival of an event.**

✅ The whole decision is now ONE pure rule — `src/input/gizmo_state.ts` — and `scene.ts` only
gathers data and draws. Three properties hold **by construction**:

1. ⭐ **Idempotent.** Calling it twice in a frame gives the same answer. The four per-frame maps it
   replaced were *consumed* on read, so a second call returned an empty gizmo.
2. ⭐⭐ **No lifetime bug is expressible.** Nothing is remembered, so nothing can fail to be cleared.
   A release, a mode toggle, a body swap and a second finger's lift are all answered by the
   **absence of state** — *a rule that has to remember to forget is what produced reports 6 and 7.*
3. ⭐ **Exhaustively testable.** `tests/gizmo_state.test.ts` runs (aligned × free) × (translate ×
   rotate) × (one × two fingers) × (moving × resting), with each of the four reports as its own
   named vector. ⛔ That matrix was **unwritable** while the decision lived in the render loop —
   which is why every one of the seven was found by a hand.

## ⚠⚠ THE ONE THING EMISSION GAVE FOR FREE, AND STATE DOES NOT

Asking *did this channel emit this frame* was an **implicit membership filter**: a channel that
drives nothing emits nothing. ⛔ A motion STATE has no such property — the second finger's `dy` reads
`MOVING` whether it is lifting the body or doing nothing at all.

⚠ That is exactly where the one regression of this pass came from: *"there cannot be green axis in
rotation mode for unaligned object."* ⭐ So membership is now **explicit** and recorded where
`secondFingerDrive` / `pinnedSecondDrive` / `translatesOnDrag` are CHOSEN between (`Held.anchorLifts`,
`Held.holderTranslates`) — never re-derived at the instrument, because a second opinion about what a
finger drives can disagree with the rule that actually ran.

## ⭐⭐ What to carry into the rest of the game

1. **A marker's POSITION must not be a function of the input.** Every property of the input becomes
   a property of the marker — lag, jitter, noise, flare. Anchor it to geometry.
2. **A readout's EXISTENCE must not be a function of this frame.** Key it on presence — *is this
   body held* — never on whether an event arrived.
3. **Do not answer a noisy source with memory.** It converts a flicker into a thing that will not go
   away, and the second defect is harder to see than the first.
4. **Lift the decision out of `scene.ts` the FIRST time it is wrong.** Five of these seven cost a
   device pass each precisely because nothing could go red.
5. ⛔ **And a green suite still does not close it** — rule 5 stands.

---

# ⛔ THE GIZMO FLICKER, HYPOTHESES 6 AND 7 — **BOTH FALSIFIED** (2026-09-24)

⚠ Kept because the way they died is the useful part; the answer is the section AFTER this one.
⛔ **6, the event rate**: killed by the owner in one sentence — *"case B with both fingers moving:
I already reported that there is no flicker"* — the report rate does not split. ⛔ **7, the
anchor's orbit under rotation**: killed by *"forget rotation, the issue exists anyway if the second
touch drives only vertical translation."* ⭐ Both were reasoned from the code and neither survived
one sentence of device evidence.

## The falsified text, as written at the time

> *"I can't reproduce the flicker on my iPhone running the github page. The flicker is on my tablet
> on usb debugging, though."* — the owner, 2026-09-24

⛔ **STATUS: UNCONFIRMED.** The owner cannot run USB debugging right now. The protocol that
confirms or kills it is in
[`../../50_BUILD_DEPLOY/DEVICE_TESTING_USB.md`](../../50_BUILD_DEPLOY/DEVICE_TESTING_USB.md).
⚠ Five hypotheses have already been wrong here. **Do not build a fix on this one until the
numbers come back.**

## ⭐⭐⭐ The mechanism, from the code

`AxisBand.tick` (`input/motion.ts`) sets an axis `STATIONARY` when `now − lastSample ≥
restConfirmMs`, **timed from the last sample that axis saw** — *silence is the evidence*.
`restConfirmMs` is **30 ms**.

⛔⛔ **So any pointer whose move events arrive more than 30 ms apart reads `STATIONARY` between
events while the finger is physically still moving.** On, off, on, off — a flicker, per pointer,
in every rule that reads the motion state. The gizmo is simply the first instrument that DRAWS
that state, which is why it is where the defect became visible.

⭐⭐ **AND IT IS THIS PROJECT'S MISTAKE SHAPE 1 IN MIRROR IMAGE.** §1.1 already paid for *a rate
estimated over too short a baseline*: the motion state once estimated speed over one sample pair,
and with 0.761 mm of noise `STATIONARY` became **unreachable**. This is the same defect the other
way round — with sparse events, `MOVING` becomes **unholdable**.

## ⭐⭐⭐ Why the owner's own description is the strongest evidence FOR it

> *"a competition between the pairs (green, grey) vs. (blue, red)"*

⛔⛔ **THOSE PAIRS ARE EXACTLY THE PER-POINTER GROUPING**, and nothing in the rules groups them
that way:

| pair | driven by |
|---|---|
| green + grey | the SECOND touchpoint's `y` and `x` |
| blue + red | the HOLDER's `y` and `x` |

⭐ A decay timed per pointer blanks both of a pointer's channels **together**, and two pointers
whose events interleave produce two pairs **alternating** — which is the reported symptom, stated
before any of this was suspected. ⚠ No rule in `gizmo_state.ts` can produce that grouping; only
the sampling can.

## ⭐⭐ Does it explain case A vs case B? — the owner's falsification test

> *"if the explanation you provided cannot explain this difference, it may not be the real issue."*

⭐ It does, **but only with one further premise, which must be tested and not assumed**:

* **Case A** (aligned body, both touches driving): *both* contacts are actively moving — one rolls
  and lifts, the other drags. A controller that splits its report rate across **moving** contacts
  halves each one's rate: ~16.7 ms → ~33 ms, **just past the 30 ms threshold**. Both pairs decay,
  alternately. ✅ Flicker.
* **Case B** (free body, translate): in ordinary play only the HOLDER moves; the second finger
  rests. ⛔ Most controllers report only contacts that **changed**, so a static finger costs the
  holder nothing and its interval stays ~16.7 ms. ✅ No flicker.

⚠⚠ **THE PREMISE THAT MUST BE TESTED IS THE WORD *MOVING*.** If the rate splits merely because two
contacts are DOWN, then case B with a resting second finger should flicker too — and it does not.
⛔ **So the killer experiment is case B with BOTH fingers moving**: if that does not flicker, this
hypothesis is dead, whatever the gaps say.

## ⚠ Why the iPhone on Pages does not show it — two variables, both changed at once

⛔ The owner's comparison changes **device AND build** together, so it cannot attribute by itself:

1. iOS delivers both contacts every frame, and its base rate is higher;
2. the Pages build is a production bundle, while USB debugging serves **unbundled dev modules with
   DevTools attached** — a renderer under 60 fps pushes the dispatch interval past 30 ms *even for
   a single pointer*, because Chrome on Android aligns `pointermove` dispatch to vsync.
   ⭐ That is why `getCoalescedEvents()` exists in the W3C Pointer Events spec at all: the browser
   delivers one dispatch per frame per pointer while the hardware samples faster.

⭐ **The protocol separates them**: run the PRODUCTION build over USB before concluding it is the
device.

## ⛔ If it is confirmed, the fix is a NUMBER, not a rule

⚠ `restConfirmMs` = 30 ms is **3 ms** below two frames at 60 Hz. That is not a threshold, it is a
coincidence. ⭐ It has a slider and a URL override, so the fix is measured, not guessed — and
`METHOD` applies: *a simulation narrows the range; it does not pick the number.*
⛔ Raising it globally changes the rest test for **every** rule, so what it costs has to be judged
by a hand too, not only what it fixes.

---

# ⭐⭐⭐ THE GIZMO FLICKER — **FOUND, AND IT WAS `A11` ALL ALONG** (2026-09-24)

> *"flips/s varies from 0,0,0,0,0,0 to x,y,z,w,0,0 with x, y, z, w somewhere between 0 and 4"*
> *"Case B with double touch actively translating the object: flips/s up to x,y,z,0,0,0 and same
> range as before."* — the owner, measuring it at last

## ⛔⛔⛔ THE MEASUREMENT DISSOLVED THE QUESTION

**Case B flips at the same rate as case A.** The only difference is grey, which case B does not
drive. ⚠ So *"case A jitters and case B does not"* — the premise behind **six** analyses, including
three of mine in this file — was a difference in how the two gestures are PERFORMED and how many
lines wink, not a difference in mechanism.

⭐⭐ `METHOD`: *a comparison between two cases is a hypothesis about the cases, not a fact about
them.* It took one counter to kill it, and the counter took ten minutes to build.

## ⭐⭐⭐ The mechanism, exactly

`AxisBand` clamps the offset to **exactly** the band boundary while an axis is moving:

```
this.offset = sign * bandPx;     // pinned ON the boundary
```

⛔⛔ So **any reversal at all** — `offset += travel` with `travel` of the opposite sign — puts it
back INSIDE the band. `restingSinceMs` starts, and `restConfirmMs` (30 ms) later that axis reads
`STATIONARY`. ⚠ A real drag curves and makes micro-corrections, so **each screen axis reverses 1–4
times a second** — precisely the measured rate, on exactly the channels the hand was driving.

⭐ The channel logic was never wrong. **The quantity it reports blinks, by design**, and every fix
I shipped was to the reporting layer above an intermittent signal.

## ⚠⚠ AND MY OWN "STRUCTURAL" FIX REMOVED THE ONLY THING BRIDGING IT

⛔ I replaced the memory with *nothing remembered* and called it robust. The memory was wrong —
it was **unbounded** — but *some* bridging is required, because the source genuinely blinks.
⭐⭐ `METHOD`: *before removing a smoothing, find out whether the signal under it is smooth.*
The persistence reports and the flicker are two ends of one dial, and I set it to zero.

## ✅ The fix: a BOUNDED hold, with three ways to go dark

`gizmoHoldMs` (200 ms, slider, `0` disables). A line is lit when its channel is being pushed, or
when it was pushed within the hold **and its driver still exists**:

| goes dark because | |
|---|---|
| the hold expired | the clock — bridges a reversal, nothing longer |
| the driver lifted | **availability**, checked every frame — a release is still INSTANT |
| the rule stopped driving it | availability again — e.g. green on a free body in `ROTATE` |

⛔ Only the first is a clock, which is what separates this from the memory that was rejected.
⭐ `instant` is returned beside `channels` so the caller stamps the clock from what is being PUSHED
— stamping from what is DRAWN would let a held line renew its own hold for ever, and the bound
would be infinite again. ✅ 12 vectors; three mutants caught (clock-only expiry, unbounded hold,
ownership by glow rather than push).

⚠⚠ **UNJUDGED**: 200 ms is a guess with a slider, and no hand has seen it. ⛔ Rule 5 unmet.
