...
| `D73` | ⭐⭐⭐ **A ROTATION TRUNCATES BACK TO THE INCREMENT IT LAST PASSED (TRIAL)** | 2026-09-22 | *"the quaternion is not rotated any further than the previous increment … smoothly truncated."* ⭐⭐ `rotationIncrementDeg` 0–45 step 5, **0 = current build**. ⛔⛔ **THREE FORMULATIONS, TWO CORRECTED BY A HAND**: quantising the turn *as it happened* lagged the finger; rounding *at the release* carried the body FORWARD of it. ⭐ The trigger is now REST (§1.1’s still/moving test, no new number) and it only gives ground BACK → `input/rotation_increment.ts` |
