...
| `D72` | ⭐⭐⭐ **THE RIGHT-HAND BODY IS A TRAPEZOIDAL PYRAMID, AND HALF AGAIN AS THICK** | 2026-09-22 | *"modify the rectangle on the right to be a trapezoidal pyramid"*, then *"increase 50% the thickness (in the x axis direction)"*. ⭐⭐ Built by MOVING a Babylon box's vertices (`core/frustum.ts`), so the builder's winding, indices and vertex splitting all survive — authoring a mesh means authoring a winding, and `D50` records what a wrong one cost. ⛔ Everything downstream inherited it for free because `D49`/`D50` had already made the hull and the faces MESH-derived: the first non-box body is what proves those two paid. ⛔⛔ **The boot clearance moved 320 → 300 mm** and the fixtures followed the product. ⚠ The boot pair now aligns on the flat **±y** faces — a frustum has no exact `+x` face, and the owner chose that over tilting the Follower by the taper angle |
| `D71` | ⭐⭐⭐ **THE SESSION BOOTS IN `TRANSLATE`** | 2026-09-22 | *"Set the default to translation mode at scene boot."* ⚠⚠ It reverses *"Default start: rotation mode"* (2026-09-16) and its 2026-09-17 re-confirmation, **both kept** — *a claim that was overturned is more useful than one silently deleted*. ⛔⛔ **It is NOT the audit's defect returning**: that was a DISAGREEMENT between every document and the code, and the tell is not the VALUE but whether any instruction still asks for the other mode. ⭐ `D23`'s old argument — *the commonest gesture on the cheapest input* — is the one that finally won |
