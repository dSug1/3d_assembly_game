...
| `D94` | ⭐⭐ **A MOUSE IS THE TOUCHPOINT IT LACKS — LMB stays the browser's, RMB is #2** | 2026-09-25 | *"propose a mapping which is user friendly."* ⛔⛔⛔ **THE FIRST BUILD REPLACED A STREAM THAT ALREADY WORKED AND FROZE IT** (defect 71): a mouse was already touchpoint #1, and intercepting every event to add one was the wrong blast radius. ⭐⭐ **PASS THROUGH BY DEFAULT, INTERCEPT BY EXCEPTION** — only the right button, `Shift`+drag and the wheel are taken. ⭐ #2 **parks** unless Shift is held, which is exact: `A11`'s deadband makes a still pointer emit **nothing**. ⛔ The wheel synthesises a **real pinch**, never a camera radius. ⚠⚠ **Playable, never testable** |
