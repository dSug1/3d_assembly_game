...
| `D88` | ⭐⭐⭐ **THE FUCHSIA OFFER — every face the held body is nearly ready to MATE with** | 2026-09-24 | ⭐ Binding, ⚠ → history |
...
| `D94` | ⭐⭐ **A MOUSE IS TWO TOUCHPOINTS — LMB is #1, RMB is #2** | 2026-09-25 | *"propose a mapping which is user friendly."* ⛔⛔ **TWO BUTTONS, NOT A MODIFIER**: `Shift`+LMB cannot hold two pointers down, and #1 staying down while #2 presses IS the alignment gesture. ⭐ The cursor drives the most recently pressed and the other **parks** — exact, not approximate: `A11`'s deadband makes a still pointer emit **nothing**. ⭐⭐ The wheel synthesises a **real pinch**, because a camera radius would be a second implementation of zoom. ⛔ A TRANSLATION, not a second input path — deleting one import and one call leaves the touch build identical. ⚠⚠ **Playable, never testable** |
...
| `D84` | ⭐⭐⭐ **`WorldAxisB` NOW GOVERNS A FREE BODY'S *ROTATION* BASIS TOO** | 2026-09-23 | ⭐ Binding, ⚠ → history |
