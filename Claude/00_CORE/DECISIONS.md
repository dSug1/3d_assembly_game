...
| `D94` | ⭐⭐ **THE RIGHT MOUSE BUTTON IS THE SECOND TOUCH** — rebuilt from scratch | 2026-09-25 | *"Do not repeat the previous build as it has continuously failed to work."* ⛔ Four rounds, all wrong in a PREMISE about the real pointer. ⭐⭐ This one never touches a DOM event: it works at Babylon's `onPrePointerObservable` seam and models ONLY #2, reading `buttons` every event. ⚠ Playable, never testable → defect 70 |
