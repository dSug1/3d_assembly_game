...
| `D38` | ⭐⭐ **FORK C IS THE DEFAULT, AND ITS ALIGNMENT NO LONGER SWITCHES THE MODE** | 2026-09-16 | Five corrections after the first device pass. ⭐ *"The game shall start by default to fork C"* — which overrules my reasoning that the default must be the only set a hand has closed; the owner is the hand. ⛔⛔ And the automatic switch to translation after an alignment is **retired — a rule the owner had dictated himself**: *"it makes the game too complicated"*, and it blocked testing the flick after an alignment. ⭐⭐ It costs an argument I liked: the tap's two meanings **coincided** while the alignment ended in `TRANSLATE`. They no longer do — **the alignment CONSUMES the tap** and `D28`'s toggle is overridden for that one gesture. ⚠ Nothing is trapped: any other tap still toggles. ⭐ Two defects of mine came with it (ledger 44, 45) → [`../10_INPUT_TOUCH/spec/FORK_C_ANCHOR_RULES.md`](../10_INPUT_TOUCH/spec/FORK_C_ANCHOR_RULES.md) |
| `D37` | ⭐⭐⭐ **FORK C IS SPECIFIED, AND ITS TRIGGER IS A TAP — no flick anywhere** | 2026-09-16 | Hold an object, **tap a face on another**, and the held one turns the minimum amount that makes its own face point **the same way** (parallel — the CAD *align* sense, the owner's choice over a mate). ⭐⭐ The owner left fork B because a release-time trigger *"releases the finger from the object it is tracking"* — **both of its faults are properties of reading a verdict at the lift**, so fork C's alignment completes mid-gesture with the object still held. ⛔ One alignment at a time (replaced, never stacked, so fork B's zero-DOF freeze cannot be built); a shake releases it in **either** mode; the rotation reset returns, scoped by *when* the alignment happened. ⚠ Rules, conflict check and the four answers → [`../10_INPUT_TOUCH/spec/FORK_C_ANCHOR_RULES.md`](../10_INPUT_TOUCH/spec/FORK_C_ANCHOR_RULES.md) |
