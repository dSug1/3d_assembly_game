...
| `D46` | ⭐⭐⭐ **APPROACH & MATE — the owner's mechanism, measured between CENTRES** | 2026-09-17 | ⭐⭐ **IT REMOVES THE BLOCKER THAT STOPPED THE PREVIOUS APPROACH**: that one mapped a finger onto the direction to a point ON a face, which **collapses at contact**; this one uses **centre → centre**, and the centres cannot meet because `MinDistanceBeforeSnapIsConfirmed` holds them a cube-length apart. ⛔ Three visible states — capture (white contours), docking (the drag partitions by ANGLE, not by mode), and an authorised MATE (anti-align + translate-snap). ⚠ **Specified, not built**: ten decisions first, of which two will bite — a mate is a SECOND stack entry (today's twist rule would free-rotate a seated part) and **eviction spares mates**, so nothing can unseat one → [`../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md`](../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md) |
