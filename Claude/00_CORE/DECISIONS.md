...
| `D80` | ⭐⭐⭐ **THE TWO `dy` CHANNELS ARE SWAPPED — the holder LIFTS, the second finger PUSHES AWAY** | 2026-09-23 | *"for translation the gravity input comes from the dy of the second touch and the depth input comes from dy of first touch. Swap."* ⭐ It restores the pairing rule 6 and `A10` had before `D75`, now expressed in **object axes**. ⛔⛔ Three things follow: the holder's plane is **{x, gravity}**, a VERTICAL one that faces the camera at every ordinary pose (the horizontal one went edge-on at a LEVEL camera); the degenerate axis is **depth**, which is where `depthTranslate`'s judged fixed-rate fallback came from; and ✅ **`D74`'s in-zone basis STOPS BEING INERT** — the plane contains `x`, which the leading face rotates |
| `D79` | ⭐⭐⭐ **THE OFFSET RADIUS ZONE IS DECOUPLED FROM THE ALIGNMENT — any body, any other body, one zone at a time** | 2026-09-23 | ⭐ Binding. It reverses `D62` and restores `A21`; the difference is a **LOCK**, and a **PRESSED PAIR OUTRANKS EVERY DISTANCE**. ⛔ Corrected the same hour: the subject must be a body **under a finger** → §22 |
| `D78` | ⭐⭐⭐ **THE ALIGNMENT IS ANTI-PARALLEL — a FollowerFace points AT the PioneerFace** | 2026-09-23 | ⭐ Binding, reversing `D37`'s parallel *align* sense; both texts stand. ⭐⭐ The sign lives in **one place** (`alignTargetFor`) and both entry points take the Pioneer's own normal. ⚠ It does not make a mate: `3D2`'s seat and a gesture are still owed → §5.18 |
| `D77` | ⭐⭐ **A FROZEN BODY SHOWS NO GIZMO, AND A SECOND TOUCH ON ONE IS A MISS** | 2026-09-23 | ⭐ Binding — both enforced at a DEFINITION rather than a guard at the point of use → `core/leading_face.ts`, `input/frozen_pick.ts` |
| `D76` | ⭐⭐⭐ **A TRANSLATION TRACKS THE FINGER — the cosine loss is a DEFECT, and the edge-on case must not go dead** | 2026-09-23 | ⭐ Binding. Three device reports, one arithmetic. ⛔ Blender never binds screen-x to a world axis and loses no cosine; the delta is **solved onto both horizontal axes** (`translatePairing=1`), with **Blender's 5° cone** falling back to `depthTranslate`'s judged rate. ⚠⚠ It makes `D74`'s in-zone basis **INERT** — an owner decision, below |
| `D75` | ⭐⭐⭐ **A TRANSLATION IS PROJECTED ONTO THE BODY'S OWN AXES, AND THE CHANNELS ARE REMAPPED** | 2026-09-22 | *"dx and dy from touch … control the translation on object x axis and object depth axis, and dy from second touch … on object gravity axis"*. ⛔⛔ One finger slides a body about its own HORIZONTAL plane and a second finger LIFTS it. ⚠⚠ **Its MAPPING is superseded by `D76`** — the unnormalised projection lost the cosine — but the channels and the **LeadingFace** gizmo stand → `input/axis_translate.ts`, `core/leading_face.ts` |
| `D74` | ⭐⭐⭐ **`WorldAxisB` — THE OBJECT AXES ARE FIXED AT SCENE BOOT, AND THE CAPTURE ZONE OVERRIDES THEM** | 2026-09-22 | ⭐ Binding, **default ON**; `0` restores the camera-referred build. ⛔ Inside the offset radius the basis is the **LeadingFace normal, gravity and their orthogonal**, orthogonalised, re-decided on the zone's **EDGE**. ⚠⚠ Under `D76`'s plane solve that basis is **INERT** — see the open row below → `input/object_axes.ts` |
...
| `D62` | ⚠⚠ **REVERSED BY `D79`** — a Follower could approach its Pioneer and nothing else; the zone is scene-wide again | 2026-09-19 | ⚠ → history. ⭐ It was itself a reversal of `A21`, so the rule has now been round the loop once — and what is different this time is the **LOCK** |
...
| `D57` | ⭐⭐⭐ **THE SECOND TOUCHPOINT'S ROLL IS FLAT — `dx`, whatever the orientation** | 2026-09-19 | ⭐ Binding — authority was `|dir.x|`, **zero** for an axis horizontal on screen, and silent → §5.5 |
...
| `D54` | ⭐⭐⭐ **`A15`'s ORPHAN UNSELECT IS DELETED — a holder keeps its object for the touchpoint's lifetime** | 2026-09-18 | ⭐ Binding — it REVERSES `D25`; **keeping control beats re-resolving** → [`queue_notes/IN8.md`](queue_notes/IN8.md) |
...
| `D51` | ⭐⭐⭐ **A HELD PIONEER MAY BE PINNED — it steers, not carries** | 2026-09-18 | ⭐ Binding — `pioneerTranslates` is a rule selector → §21 |
...
| `D49` | ⭐⭐⭐ **THE CAPTURE IS A SURFACE OFFSET, COMPUTED AT SPAWN** | 2026-09-18 | ⭐ Binding — nothing in it reads a normal, so **inverted normals cannot affect it**. ⛔⛔ The DISTANCE moved to surfaces; the approach DIRECTION must NOT → §19 |
...
| `D47` | ⭐⭐⭐ **A MATE IS BROKEN BY PULLING IT APART WITH TWO FINGERS** | 2026-09-17 | ⭐ Binding — past a `BreakThreshold` (slider). ⛔ It needs `3D2`'s **seat** first, or *breaking* is indistinguishable from *moving* → §8 |
...
| ✅ **~~THE IN-ZONE BASIS IS INERT UNDER THE PLANE SOLVE~~** — **ANSWERED BY `D80`**, and by arithmetic rather than by a choice: the swap put the holder's plane on {x, gravity}, and the leading face rotates `x`, so the basis bites again. ⚠ Kept as a correction, not deleted | `D76` made the holder's delta a **solve onto the body's horizontal plane**, and `D74`'s in-zone basis is *LeadingFace normal, gravity, orthogonal* — **orthogonalised**, so it spans that same plane and changes nothing at the zone edge. ⚠ The dictation's *"the translation direction differs inside the offset radius zone"* is therefore not in force. ⭐ Either **accept it**, or go back to `translatePairing=0` where the basis bites, or let the in-zone plane **TILT with the face** (the literal reading, rejected on my advice) — the only option where the zone rule and finger-tracking both hold → [`queue_notes/IN4.md`](queue_notes/IN4.md) |
