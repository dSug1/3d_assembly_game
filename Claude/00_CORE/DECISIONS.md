...
| `D59` | ⭐⭐⭐ **AN ALIGNED FOLLOWER GIVES THE SECOND TOUCH BOTH AXES** | 2026-09-19 | *"whatever translation mode, when an object is aligned as follower the second touch shall control the depth and the roll (as this is currently the case when the second touch hit the Pioneer object)."* ⛔⛔ The rule moved from *where the finger landed* to *what the body is*: an aligned body has **one rotational DOF left**, so a mode has nothing to choose between. ✅ It settles `D58`'s `A16` collision for aligned bodies; ⚠ it survives on a **free** one → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.7 |
| `D58` | ⭐⭐⭐ **THE MOVEMENT MODE TOGGLES ON A PRESS TOO** | 2026-09-19 | *"a new continued press (= a tap where there is no release) outside any object — … or on the exact same PioneerFace."* ⭐⭐ `D55`'s sweep reaching `D28`, the last rule that still demanded a release. ⚠⚠ **Collides with `A16`** — the OUTSIDE finger is the one that drives depth/roll, so its channel now **alternates** instead of being chosen. Flagged, owed a device look → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.6 |
...
| `D49` | ⭐⭐⭐ **THE CAPTURE IS A SURFACE OFFSET, COMPUTED AT SPAWN** | 2026-09-18 | *"I want to modify that to an offset to the faces… I would prefer [the game computes it] as this avoids to duplicate work in Blender."* ⭐⭐ Computed wins on its own merits: nothing reads a normal, so **inverted normals cannot affect it**, and a phantom would be a second source of truth for one fact. ⛔⛔ **The DISTANCE moved to surfaces; the approach DIRECTION must NOT** — `D46` exists because a face-to-face direction collapses at contact. ⭐ Offset in **millimetres on the glass**, with a slider; the white contour **is** that shell → [`../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md`](../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md) §19 |
...
| `D46` | ⭐⭐⭐ **APPROACH & MATE — the owner's mechanism, measured between CENTRES** | 2026-09-17 | ⭐⭐ It removes the blocker that stopped the previous approach: that one mapped a finger onto the direction to a point ON a face, which **collapses at contact**; centres cannot meet. ⛔ Three visible states — capture (white contours), docking (the drag partitions by ANGLE, not by mode), authorised MATE — and **one finger on each object** drives the fine approach, which is `D47`'s break with the sign reversed. ⚠ Specified, not built; the owner answered every question the same day → [`../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md`](../10_INPUT_TOUCH/spec/APPROACH_AND_MATE.md) |
