...
| `D79` | ⭐⭐⭐ **THE TWO `dy` CHANNELS ARE SWAPPED — the holder LIFTS, the second finger PUSHES AWAY** | 2026-09-23 | *"When in translation mode and when in rotation mode with two touches pressed with object aligned: swap the inputs dy of second touch and dy of first touch."* ⭐ Those are exactly the configurations in which the holder's `dy` translates, so the remap is unconditional and observable only there. ⛔⛔ It restores the pairing rule 6 and `A10` had before the object axes, now in OBJECT axes: the holder's plane becomes **{x, gravity}**, a VERTICAL one that faces the camera at every ordinary pose — where the horizontal plane it replaced went edge-on at a LEVEL camera. ⭐ The degenerate axis is now **depth**, which is where `depthTranslate`'s judged fixed-rate fallback came from. ⚠ And a degenerate plane **degrades to a projection** rather than suppressing, which is the freeze-at-the-contour this swap caused the first time it shipped |
