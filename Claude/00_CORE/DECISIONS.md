...
| `D70` | ⭐⭐⭐ **A MOVED PIONEER COSTS A FOLLOWER WHAT A TURNED ONE DOES — cyan BREAKS, orange follows** | 2026-09-21 | *"a translation of the pioneer should break the alignment of the cyan."* ⛔⛔ It corrects `D69`, which took a CONTRAST for a rule. ⭐⭐ The real rule was already there: a `SNAPSHOT` is a copy taken ONCE, and **position and orientation are two components of one pose** — answering differently for each WAS the asymmetry. ⚠ `pioneerMoved` now sits beside `pioneerTurned` with the same three verdicts → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.17 |
