...
| `D65` | ⭐⭐⭐ **A SECOND TOUCH RELEASES; IT DOES NOT TAP — only a tap toggles the mode** | 2026-09-21 | *"Only a tap (or double-tap …) shall toggle the mode. Not a release anywhere."* ⛔⛔ A touchpoint pressed while another finger already carries a body is a **control finger**: its lift never toggles — outside any object OR on the Pioneer, driving or not. ⭐ The fact is *what the touchpoint IS*, latched at its press, because a brief control press and a deliberate tap are the same physical event. ⚠ **Cost**: `D58`'s *"a tap outside any object"* no longer toggles while holding; the capability lives on the PRESS → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.12 |
| `D64` | ⚠ **SUPERSEDED BY `D65` THE SAME DAY** — *driving consumes the toggle*; the seam and the `D61` finding survive, the FACT does not | 2026-09-21 | ⚠ → history |
