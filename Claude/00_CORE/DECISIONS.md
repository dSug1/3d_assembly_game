...
| `D69` | ⭐⭐⭐ **A TRANSLATED PIONEER CARRIES EVERY FOLLOWER, DOWN THE CHAIN** | 2026-09-21 | *"a translation of pioneer controls the same translation of all the follower objects."* ⛔⛔ **ALL of them, not only the orange** — the owner's own contrast, one clause apart, and the reading that makes `D67`'s multi-select a GROUP MOVE. ⭐ A state comparison like the turn cascade (`PioneerRef.position` mirrors `orientation`), never a delta routed from a gesture — a Pioneer moves by a finger, by depth, by a snap or by its own Pioneer → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.16 |
| `D68` | ⭐⭐ **A DOUBLE TAP REVERTS THE MODE EVEN WHEN THE SECOND HALF NEVER LIFTS** | 2026-09-21 | *"if i double tap without release the pioneer and press the follower → orange, the translation/rotation mode toggles: it should not."* ⭐ An invariant break, not a new rule: `D28`'s *two taps revert* was keyed to the second RELEASE, which `D67`'s orange never makes. ⛔ The completing PRESS takes that toggle over, and its own release is spent → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.15 |
| `D67` | ⭐⭐⭐ **THE ROLES ARE INVERTED — FIRST TOUCH THE PIONEER, SECOND THE FOLLOWER** | 2026-09-21 | *"First the Pioneer & PioneerFace, second the Follower & the FollowerFace"*, and *"to reach the orange, the first touch shall be double tap without final release"*. ⭐⭐ It buys **several Followers in one hold** — the links index was always a set per Pioneer, so only the transient grip's face-clearing had to change ends. ⛔ The subsequent touch logic is untouched: `pinnedPair` reads the links, never the press order. ⚠⚠ **Cost**: a frozen body can never be a Follower, so *align a part to the plate* is now *hold the plate FIRST* → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.14 |
| `D66` | ⭐⭐⭐ **A PRESS DOES NOT TOGGLE THE MODE — ONLY A TAP DOES, WHICH IS `D28` AGAIN** | 2026-09-21 | *"A press never toggles while a body is held, but a tap by the second touchpoint can (as per present rule for tap)."* ⛔⛔ It repeals `D58`, `D61`, `D64` and `D65` — one chain, whose first link was the only decision in it. ⭐⭐ `D58`'s press toggle fired on an aligned Follower and `translatesOnDrag` **masked it for the whole two-finger phase**, so a hand could only report it at the lift — which sent two fixes at the release. ⚠ *When a rule's effect is masked during a gesture, the reported event is where it became VISIBLE, not where it happened.* ⭐ Restores `A16`: **switching requires a tap** → [`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) §5.13 |
