...
| `D82` | ⛔⛔⛔ **THE IN-ZONE BASIS IS DELETED — inside the capture zone is the same as outside** | 2026-09-23 | *"eliminate this rule: Inside the offset radius the axes are the LeadingFace normal, gravity, and their orthogonal. Inside shall be the same as outside. I think this is polluting the approach movement."* ⛔ It reverses `D74`'s part C, dictated the day before. ⭐⭐ **Three device reports in one day were about the SWITCH, not the geometry** (defects 59, 62, 63) — each fix made it better behaved, none made it invisible, which is what a hand asks. ⭐ `METHOD`: *a rule whose every defect is about the MOMENT it takes effect is a rule about the wrong thing.* ⚠ `leadingFace` and its gizmo survive. ✅ −11 vectors |
| `D79` | ⭐⭐⭐ **THE TWO `dy` CHANNELS ARE SWAPPED — the holder LIFTS, the second finger PUSHES AWAY** | 2026-09-23 | *"When in translation mode and when in rotation mode with two touches pressed with object aligned: swap the inputs dy of second touch and dy of first touch."* ⭐ Those are exactly the configurations in which the holder's `dy` translates, so the remap is unconditional and observable only there. ⛔⛔ The holder's plane becomes **{x, gravity}**. ⚠⚠ The claim written here — *"a vertical plane faces the camera at every ordinary pose"* — is **FALSE and was never measured**: `worldAxisB` freezes the axes at boot, so a quarter turn of orbit puts `x` along the view (defect 56). ⭐⭐ And the swap itself broke almost nothing — it **separated** two axes that had travelled together, which made a day-old mis-assignment observable (defects 59, 62) |
| `D78` | ⭐⭐⭐ **THE ALIGNMENT IS ANTI-PARALLEL — a FollowerFace points AT the PioneerFace** | 2026-09-23 | ⭐ Binding, reversing `D37`'s parallel *align* sense; both texts stand. ⭐⭐ The sign lives in **one place** (`alignTargetFor`) and both entry points take the Pioneer's own normal → §5.18 |
...
| `D76` | ⭐⭐⭐ **A TRANSLATION TRACKS THE FINGER — the cosine loss is a DEFECT, and the edge-on case must not go dead** | 2026-09-23 | ⭐ Binding. Three device reports, one arithmetic. ⛔ Blender never binds screen-x to a world axis and loses no cosine; the delta is **solved onto a plane** (`translatePairing=1`), with **Blender's 5° cone** falling back to `depthTranslate`'s judged rate |
| `D75` | ⭐⭐⭐ **A TRANSLATION IS PROJECTED ONTO THE BODY'S OWN AXES, AND THE CHANNELS ARE REMAPPED** | 2026-09-22 | *"dx and dy from touch … control the translation on object x axis and object depth axis, and dy from second touch … on object gravity axis"*. ⚠⚠ Its MAPPING is superseded by `D76` (the projection lost the cosine) and its channels by `D79`'s swap; the **LeadingFace** gizmo stands → `input/axis_translate.ts`, `core/leading_face.ts` |
