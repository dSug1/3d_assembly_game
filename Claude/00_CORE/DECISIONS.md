...
| `D73` | ⭐⭐⭐ **A ROTATION *ENDS* ON A MULTIPLE OF THE INCREMENT (TRIAL)** | 2026-09-22 | *"any rotation stops at a degree which is a multiple of the incrmt … Slerp the rotation at the end."* ⭐⭐ `rotationIncrementDeg` 0–45 step 5, **0 = the current build**. ⛔⛔ **THE SECOND FORMULATION — the first was BUILT, DRIVEN AND REJECTED** for lagging the finger: it quantised the turn *as it happened*. ⭐ Now nothing is quantised DURING the drag — one slerped correction at its end, so the lag is unreachable rather than reduced → `input/rotation_increment.ts` |
