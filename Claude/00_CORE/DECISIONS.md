...
| `D92` | ⚠⚠ **REVERSED THE SAME DAY — `objectD` IS A CUBOID AGAIN** | 2026-09-25 | *"remove the hollow cylinder and reinstate the previous rectangle."* ⛔ `core/ring.ts` and its 10 vectors are **deleted, not parked** (`D28`/`D40`: *a dormant fork is a trap*). ⭐ Kept because of what the hour bought — **defect 68**: the mesh was correct and the body still read as a doughnut, because the wall and the end face share their rim vertices and `ComputeNormals` blends across them. ⭐⭐⭐ *A vector suite that reads the geometry cannot see the SHADING, and the shading is what a hand judges* — ten green vectors on a body the owner rejected on sight |
