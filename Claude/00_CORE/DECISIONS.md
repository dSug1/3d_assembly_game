...
|---|---|
| ⛔⛔ **Does a PINCH on an object move it in DEPTH?** — it re-opens `D10` | The `3D1` device pass asked how a hand gets depth, and the answer was neither option on offer: *"two fingers touchpoint on one object or two objects and pinch movement to zoom the one or two objects out."* ⭐ Strong: the metaphor already transfers (pinching out makes things smaller either way), it needs NO new discriminator (§4 already branches on *did it hit an object*), the gain is **computable** like rule 6's, and the two-object half is **already §4 rule 6ter** — now reached for unprompted, before being built. ⛔ But the ONE-object half needs *two touchpoints on the same object* to mean something, and `D10` says **ignore the second** — decided after a device look, and its accepted consequence (*the part stops responding*) would go with it. `IN8` recorded reading 2 as **deferred, not rejected** → [`queue_notes/IN8.md`](queue_notes/IN8.md) |
