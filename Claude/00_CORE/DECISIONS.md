...
⚠ A superseded row's consequence cell is therefore just **⚠ → history**. It said so in a
sentence eleven times over; the sentence is here once instead (compressed 2026-09-18 to make
room for `D49`, which is what the ratchet is for). ⭐ Compressed again 2026-09-21: the rows'
repeated LINK to that file lives here once. **An addition that does not fit pays for itself.**

⭐ **AND `§N (approach)` POINTS AT** the approach spec.
⭐ **AND A BARE `§N` IN A CONSEQUENCE CELL POINTS AT THE ALIGNMENT SPEC**,
[`../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md`](../10_INPUT_TOUCH/spec/ALIGNMENT_RULES.md) — written out
twelve times over, and once here instead (compressed 2026-09-22 to make room for `D71`/`D72`).
**An addition that does not fit pays for itself.**
...
| `D73` | ⭐⭐⭐ **A ROTATION IS ALWAYS ON AN INCREMENT (TRIAL)** | 2026-09-22 | *"stop to an increment and not rotate further if the delta position input becomes too weak."* ⭐⭐ `rotationIncrementDeg` 0–45/5, **0 = current build**. ⛔⛔ **FOUR FORMULATIONS, THREE REJECTED BY A HAND**: quantising as it happened LAGGED; rounding at the release went FORWARD of the finger; truncating on rest REVERSED — all three let the body hold a pose it was not allowed, then argued about the way back. It now never leaves an increment, advancing several at once so no backlog exists → `input/rotation_increment.ts` |
| `D72` | ⭐⭐⭐ **THE RIGHT-HAND BODY IS A TRAPEZOIDAL PYRAMID, AND HALF AGAIN AS THICK** | 2026-09-22 | *"modify the rectangle on the right to be a trapezoidal pyramid"*, then *"increase 50% the thickness (in the x axis direction)"*. ⭐⭐ Built by MOVING a Babylon box's vertices (`core/frustum.ts`), so the builder's winding survives — `D50` records what a wrong one cost. ⛔ Everything downstream inherited it free: `D49`/`D50` had already made the hull and the faces MESH-derived. ⛔⛔ **The boot clearance moved 320 → 300 mm** and the fixtures followed the product. ⚠ The boot pair aligns on the flat **±y** faces: a frustum has no exact `+x` face |
