...
| `D73` | ⭐⭐⭐ **A ROTATION IS ALWAYS ON AN INCREMENT (TRIAL)** | 2026-09-22 | *"stop to an increment and not rotate further if the delta position input becomes too weak."* ⭐⭐ `rotationIncrementDeg` 0–45/5, **0 = current build**. ⛔⛔ **FOUR FORMULATIONS, THREE REJECTED BY A HAND**: quantising live LAGGED; rounding at the release went FORWARD of the finger; truncating on rest REVERSED — all three let the body hold a pose it was not allowed, then argued about the way back. It never leaves one now, advancing several at once so no backlog can exist |
