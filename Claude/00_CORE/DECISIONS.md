...
| `D79` | ⭐⭐⭐ **THE OFFSET RADIUS ZONE IS DECOUPLED FROM THE ALIGNMENT — any body, any other body, one zone at a time** | 2026-09-23 | *"No need for a pioneer-follower relationship … one offset radius zone at a time, based on closest faces twins"*. ⛔⛔ It **reverses `D62`** and restores `A21`, and the difference is a **LOCK**: a pair that has entered keeps the zone until it leaves, so a nearer third body cannot steal it — `D62` was fixing a pair that kept re-choosing itself. ⭐⭐ **A PRESSED PAIR OUTRANKS EVERY DISTANCE** (`D67`'s gesture kept as a DESIGNATOR while it stops being a REQUIREMENT). ⚠ The threshold is still `D49`'s HULL gap and the **face twins** are computed for the named pair. ⛔⛔ **CORRECTED THE SAME HOUR**: the subject must be a body **under a finger** — *"when no object is touched, there cannot be any white highlight"* — so it is subject × scene, not scene × scene → §22 |
...
| `D77` | ⭐⭐ **A FROZEN BODY SHOWS NO GIZMO, AND A SECOND TOUCH ON ONE IS A MISS** | 2026-09-23 | ⭐ Binding — both enforced at a DEFINITION rather than a guard at the point of use: `leadingFace` refuses a frozen body, and the pick is filtered **before** `IN2`'s latch. ⛔ The FIRST touch is untouched → `core/leading_face.ts`, `input/frozen_pick.ts` |
