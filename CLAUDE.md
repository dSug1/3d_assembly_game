...
✅ Green: TypeScript + Babylon + Vite, **1101 golden vectors passing** — ⚠ MEASURED 2026-09-23; the
...
⛔⛔⛔ **AND THE FULL CHECK THE OWNER THEN DEMANDED FOUND THE REAL PAIR** (defects 62–63, 2026-09-23): the zone edge **replaced the basis without looking at the old one** — `previous` was passed in and used only as a refusal — so at the crossing every channel's axis could turn 90° or reverse, and the finger that was advancing the body handed the approach to the other channel: *"blocked on white highlight border, change of directions"*. ⭐⭐ **The mate GEOMETRY is not negotiable, its ORIENTATION is**: the zone now adopts the framing nearest the basis the body arrived with, so nothing turns more than 45° and **the channel doing the approach keeps it**. ⛔ And the fixed-rate fallback aimed itself by `sign(towardGravity)`, a camera ELEVATION, while the exact branch aimed by the axis's screen shadow — *"inversion of dy input direction"*. ⭐ The fallback now replaces only the RATE, so it **cannot invert by construction**. ⚠⚠ A vector had claimed that continuity since the rule shipped and COULD NOT FAIL: it was built on the camera's own axes, where the two sign sources are the same number — *a fixture chosen because it is easy to reason about is usually chosen from the set where the quantity under test is ZERO*, the fifth time.
...
⛔⛔ **Sixty-six defects, sixty-three of them BY FINGER, and none visible to a green
