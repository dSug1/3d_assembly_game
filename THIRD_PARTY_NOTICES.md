...
| **Hyper circle fit** | A. Al-Sharadqah & N. Chernov, *"Error analysis for circle fitting algorithms"*, Electronic J. Statistics **3** (2009) 886–911, [arXiv:0907.0421](https://arxiv.org/abs/0907.0421) | ✅ Published mathematics — no licence attaches to a formula and no patent is asserted. Independent implementation from the paper's algebraic form. `N13`: clear for commercial use. | `src/input/roll.ts` — estimates the centre the roll angle is measured about ⛔⛔ **AND THE CODE IS DELETED (2026-09-16)** — `A12` moved roll to the second touchpoint's x, which needs neither a circle fit nor a smoother, and the unwired detector then caused defect 40. ⭐ The attribution stays: it shipped, and this file records what was used, not only what is used. |
| **1€ filter** | Casiez, Roussel & Vogel, CHI 2012, doi [10.1145/2207676.2208639](https://dl.acm.org/doi/10.1145/2207676.2208639) | ✅ Reference implementations at <https://gery.casiez.net/1euro/> are **BSD**/**MIT**, **no patent asserted**; ours is an independent implementation from the paper. `N13`-clear. | `src/input/one_euro.ts` — ⭐ **SHIPPED, smoothing the roll angle** (`beta = 0`), on a device A/B that overturned my own metric ⛔⛔ **AND THE CODE IS DELETED (2026-09-16)** — `A12` moved roll to the second touchpoint's x, which needs neither a circle fit nor a smoother, and the unwired detector then caused defect 40. ⭐ The attribution stays: it shipped, and this file records what was used, not only what is used. |
| **Rigid-body drag under a force** (mass-spring-damper) | NVIDIA **PhysX**, the engine behind Unity's 3D physics — integration and linear damping in `DyBodyCoreIntegrator.h`; classical mechanics otherwise | ✅ PhysX has been **open source under BSD-3-Clause since 4.0 (Dec 2018)**, later extended to the GPU source; no patent asserted. ⛔ **No code was taken and none needed to be**: what is reused is the MODEL (`ẍ = −ω²(x−target) − 2ζωẋ`, i.e. `AddForce` with `linearDamping`), which is Newton. `N13`-clear. | `src/input/follow.ts` — rule 6's inertia. ⭐ **The model is Unity's; the ARITHMETIC deliberately is not.** PhysX takes a semi-implicit Euler step and damps by `(1 − c·dt)`, which is timestep-DEPENDENT — raised upstream as a flaw, and survivable for Unity only because it runs physics at a fixed 0.02 s step. We integrate on a stuttering render frame, so ours is the exact analytic solution. A vector measures the two against each other at Unity's own timestep, and shows the gap is PhysX's discretisation error by watching it vanish as its step shrinks. |
...
| **Critically damped follower on SO(3)** (rotation inertia + phantom "slerp") | Classical mechanics on the rotation vector of the orientation error; same mass-spring-damper as `follow.ts` | ⛔ **BUILT, 12 vectors green, and REJECTED BY THE OWNER ON THE DEVICE**: *"I did not like the rotation inertia and slerp implementation."* Rotation stays direct. ⭐ The engineering is worth keeping in mind if it is ever revisited: it ran in the ERROR space, not on the orientation, because a follower on `log(orientation)` chases the ±π branch cut and spins the object the long way round once, mid-gesture — a defect no small test reveals. ⚠ Translation has inertia and rotation does not; they were judged separately and came out differently. No licence question either way. |
